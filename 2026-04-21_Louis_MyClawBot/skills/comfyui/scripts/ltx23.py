...
def _ltx2_loaders(g):
    ...
    For VAE: uses CheckpointLoaderSimple for video VAE and
    LTXVAudioVAELoader for audio VAE (both from ltx-2.3-22b-dev-nvfp4.safetensors).
    """
    ...
    clip  = g.node("DualCLIPLoader",
                   ...
                   type="ltxv")
    # Load the full model checkpoint to get video VAE
    checkpoint = g.node("CheckpointLoaderSimple",
                       ckpt_name="ltx-2.3-22b-dev-nvfp4.safetensors")
    # checkpoint[0] = MODEL, checkpoint[1] = CLIP, checkpoint[2] = VAE
    video_vae = checkpoint[2]  # VAE output (for video encode/decode)
    # Load audio VAE separately from the same full model
    audio_vae = g.node("LTXVAudioVAELoader",
                       ckpt_name="ltx-2.3-22b-dev-nvfp4.safetensors")
...
def _ltx2_decode(g, latent_out, video_vae, audio_vae=None):
    ...
    if audio_vae is not None:
        ...
        images = g.node("LTXVSpatioTemporalTiledVAEDecode", vae=video_vae, latents=sep[0],
        ...
        audio  = g.node("LTXVAudioVAEDecode", samples=sep[1], audio_vae=audio_vae)
        return images, audio, sep[1]
    else:
        images = g.node("LTXVSpatioTemporalTiledVAEDecode", vae=video_vae, latents=latent_out,
                        spatial_tiles=4, spatial_overlap=1,
                        temporal_tile_length=16, temporal_overlap=1,
                        last_frame_fix=False, working_device="auto", working_dtype="auto")
...
def _ltx2_second_pass(g, model, cond, images, audio_latent,
    ...
    video_lat_2  = g.node("LTXVImgToVideoInplace", vae=video_vae, image=upscaled[0],
...
def ltx2_image_to_video(image_filename, prompt, seconds=3, fps=24, camera_lora=None,
    ...
    video_latent = g.node("LTXVImgToVideoInplace", vae=video_vae, image=prep[0],
...
def ltx2_multiframe(guide_frames: list[tuple[str, int, float]], prompt,
    ...
    for img_file, frame_idx, strength in guide_frames:
        ...
        guided = g.node("LTXVAddGuide", positive=cur_pos, negative=cur_neg,
                        vae=video_vae, latent=cur_latent, image=prep[0],
