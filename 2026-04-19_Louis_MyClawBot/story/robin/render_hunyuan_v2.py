"""
HunyuanVideo T2V 渲染 - 修复 CLIP 链接问题
DualCLIPLoader 给 positive 用，单独的 CLIPLoader(umt5) 给 negative 用
"""
...
def build_hunyuan_t2v(prompt_pos: str, width=1280, height=720, num_frames=121, seed=42, steps=8, cfg=7.0):
    ...
    # 双CLIP：给 CLIPTextEncodeHunyuanDiT 用（bert + mt5xl）
    ...
    # Positive conditioning - Hunyuan 专用
    cond_pos = g.node("CLIPTextEncodeHunyuanDiT", clip=dual_clip,
        bert=prompt_pos, mt5xl=prompt_pos)

    # Negative conditioning - 也用 DualCLIPLoader（整个对象，不是 [0]）
    cond_neg = g.node("CLIPTextEncode", text=NEGATIVE, clip=dual_clip)
...
if __name__ == "__main__":
    ...
    print(f"Building HunyuanVideo workflow (v2 - fixed CLIP)...")
    print(f"  Prompt: {PROMPT[:80]}...")
    print(f"  Resolution: 1280x720, Frames: 121, Steps: {8}, CFG: {7.0}, Seed: {seed}")
    print()
