"""
Shot 4 三模型对比 v3 - 最终修复版
"""
import sys, io, json, time, urllib.request, urllib.error, threading

LOG = open(r'C:\Users\user\V915-31\.openclaw\workspace\story\robin\run_compare2.log', 'w', encoding='utf-8', buffering=1)
def log(msg):
...
def build_wan22(prompt_pos, width=1280, height=720, num_frames=121, seed=None, steps=8):
    ...
    g.node("SaveVideo", video=video, filename_prefix="cmp3_wan22", format="mp4", codec="auto")
    return g.to_dict()

# ── HunyuanVideo (FIX v2: use clip_raw directly) ──────────────
def build_hunyuan(prompt_pos, width=1280, height=720, num_frames=121, seed=None, steps=8, cfg=7.0):
    ...
    if seed is None: seed = int(time.time()*1000) % (2**32)
    g = WorkflowGraph()
    # DualCLIPLoader with hunyuan_video type - use clip_raw directly (not clip_raw[0])
    ...
    g.node("DecodeAndSaveVideo", video_latent=denoised, fps=24.0,
           filename_prefix="cmp3_hunyuan", format="auto", codec="auto",
           video_vae=vae, tiling="disabled")
    return g.to_dict()

# ── LTX 2.3 (v2 - skip audio VAE, only video) ──────────────
def build_ltx(prompt, seconds=5, fps=24, seed=None):
    ...
    # Skip audio VAE to avoid JSON decode issues
    ...
    g.node("SaveVideo", video=video[0], filename_prefix="cmp3_ltx", format="auto", codec="auto")
    return g.to_dict()
...
def wait(label, prompt_id, timeout=TIMEOUT):
    ...
    while time.time() < deadline:
        try:
            ...
            # history endpoint returns {prompt_id: entry}
            ...
            if st == "success" and outputs:
                ...
                for m in msgs:
                    if 'exception_message' in str(m):
                        log(f"[{label}] ERR: {str(m)[:200]}")
                return "ERROR"
...
log(f"WAN22={len(wan_wf)} HUNYUAN={len(hun_wf)} LTX={len(ltx_wf)}")
