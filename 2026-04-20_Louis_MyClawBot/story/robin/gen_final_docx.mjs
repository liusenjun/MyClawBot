...
function makeShotTable(shots) {
  const headerRow = new TableRow({
    tableHeader: true,
    children: ['#', '时间', '场景', '类型', '叙事功能', '摄像机', 'Prompt', '参考图', '音效/情绪'].map(h => new TableCell({
...
const doc = new Document({
  sections: [{
    ...
    footers: {
      default: new Footer({
        children: [new Paragraph({ alignment: AlignmentType.RIGHT, children: [new TextRun({ text: 'Generated by 小志 x OpenClaw', size: 16, color: '888888' })] })]
      })
    },
    children: [
      ...
      new Paragraph({ children: [new TextRun({ text: `共 ${storyboard.shots.length} 个镜头 | 完整 Prompt（英文）`, size: 18, color: '666666' })], spacing: { before: 0, after: 200 } }),
...
Packer.toBuffer(doc).then(buf => {
  ...
  console.log('Done: storyboard_v2_final.docx');
});
